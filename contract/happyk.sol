...
contract InvestorRelationship is Config, SeroInterface {
    ...
    string public constant FPRS_CURRENCY = "FPRS";
    string public constant TOKEN_CURRENCY = "HAPPYK";
    ...
    function _balanceOfSero() internal view returns (uint256){
        return sero_balanceOf(FPRS_CURRENCY).sub(cash);
    }
...
contract HappyK is InvestorRelationship, Ownable {

    uint256 private constant FIVEHUNDREDTHOUSAND = 5e24;
    uint256 private constant TENMILLION = 1e25;
    uint256 private constant HUNDREDMILLION = 1e26;

    uint256[] private  dList = [1e17, 2e17, 3e17, 4e17, 12e17, 2e18, 4e18, 8e18, 102e17];
    uint256[] private  startList = [8e17, 28e17, 68e17, 128e17, 208e17, 448e17, 848e17, 1648e17, 3248e17];
    ...
    function conversionRate() public view returns (uint256, uint256) {
        if (allIncome < TENMILLION) {
            return (1e18, 1e19);
        }

        uint256 m = allIncome / HUNDREDMILLION;
        if (m >= 8) {
            m = 8;
        }
        uint256 d = dList[m];
        uint256 start = startList[m];
        uint256 n = (allIncome - m * HUNDREDMILLION) / FIVEHUNDREDTHOUSAND;

        return (start + d * n, 1e19);
    }
